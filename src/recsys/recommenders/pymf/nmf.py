...
class NMF():
    ...
    def __init__(self, data, num_bases=4, seed = None, **kwargs):
    ...
    def frobenius_norm(self):
        ...
        if hasattr(self,'H') and hasattr(self,'W') and not scipy.sparse.issparse(self.data):
            ...
            err_tmp = (tmp * tmp.T).sum()
            err = np.sqrt(err_tmp)
            rmse_tmp = tmp[self.data.nonzero()]
            rmse_tmp = (rmse_tmp * rmse_tmp.T).sum()
            rmse = np.sqrt(rmse_tmp/len(self.data.nonzero()[0]))
        else:
            err = -123456
            rmse = -1
    ...
    def factorize(self, niter=100, show_progress=False,
        ...
        for i in xrange(niter):
            ...
            if compute_err:
                self.ferr[i], self.rmse[i] = self.frobenius_norm()
                self._logger.info('Iteration ' + str(i+1) + '/' + str(niter) +
                ' FN:' + str(self.ferr[i]) + ", RMSE:" + str(self.rmse[i]))
            else:
                self._logger.info('Iteration ' + str(i+1) + '/' + str(niter))
